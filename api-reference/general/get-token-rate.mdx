...
**`providerIds` is always an array**—`[]` when no provider is available, never `null`. It carries **exactly one** id when you pass `?provider_id=`, when `?limit=1`, or when `orderType` is **`otc`**. Fewer than 3 ids simply means fewer eligible providers for that corridor and notional. The **fallback provider is never listed**.
...
Routing is **optimistic**: the queue is resolved **once, at order creation**, rather than re-ranked on every retry. Two ways to use that:
...
Queues are built for **open, standard-tier** providers only. Pinned, restricted, and OTC quotes return a single id and keep the pre-existing single-provider behavior.
