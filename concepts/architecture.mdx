...
Alt text: Diagram showing the flow from Payment Intent, through Aggregator nodes, to Provision nodes, and finally to end users. Aggregator nodes coordinate between the payment intent and a network of provision nodes, which then distribute funds to recipients.
...
<Steps>
  ...
  <Step title="Order Validation">
    Contract validates parameters and sender authorization
  </Step>
  <Step title="Event Emission">
    Order event emitted for aggregator monitoring
  </Step>
  <Step title="Provider Assignment">
    Aggregator assigns order to suitable provider
  </Step>
...
The protocol uses a microservices architecture for modularity and scalability.

#### Service Components

- **Order Service**: Handle order creation and management
- **Provider Service**: Manage provider registry and rates
- **Settlement Service**: Handle settlement coordination
- **Compliance Service**: Manage KYC and regulatory compliance
- **Notification Service**: Handle webhooks and notifications
- **Analytics Service**: Process transaction analytics
...
- **Event Streaming**: Order lifecycle events
