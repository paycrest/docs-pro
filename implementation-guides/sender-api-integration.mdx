...
| Eligible providers only | Entries must be **open, standard-tier** providers serving this corridor. **Restricted** and **fallback** providers are rejected—pin those with `providerId` instead. |
...
| `provider AbCdEfGh cannot serve this order` | Entry is inactive, restricted, or has no live offer for this corridor |
