...
Pinned (`?provider_id=`) and **OTC** quotes still return exactly one id. The fallback provider is never listed.
...
| Max 3 entries, no duplicates, no empty strings | **400** |
| Format | Exactly 8 alphabetic characters per id |
| Eligibility | Open, standard-tier providers serving the corridor. **OTC**, restricted, and fallback providers are rejected — pin those with `providerId` |
| Scope | Offramp only. Onramp keeps the singular `providerId` |
