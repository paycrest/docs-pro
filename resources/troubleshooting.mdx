...
<AccordionGroup>
  ...
  <Accordion icon="danger" title="Order Creation Errors">
    ...
    <p>The recipient object is capped at <strong>500 bytes</strong> of JSON before encryption. The size check budgets for a full 3-provider routing queue on <strong>every</strong> order — including orders that send no <code>providerIds</code> — because the aggregator may resolve one onto the order and it rides in the message hash at gateway submit.</p>
    <ul>
      <li>Effective budget is <strong>~49 bytes smaller</strong> than it was before optimistic routing (<strong>~41</strong> when you pin a <code>providerId</code>). Payloads that used to pass can now fail</li>
