...
| Field | Description |
|-------|-------------|
| `providerId` | Pin the order to exactly one provider. 8 alphabetic characters. |
| `providerIds` | Ordered **provider queue** (max 3). Assignment tries entries in order. Typically the ids from [`GET /v2/rates`](/api-reference/general/get-token-rate). |
...
Omit both and the aggregator resolves a queue itself at order creation. Entries must be **open, standard-tier** providers—OTC, restricted, and fallback providers must use the singular `providerId`.
