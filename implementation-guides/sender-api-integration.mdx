...
response=$(curl -s -w "%{http_code}" -X POST "https://api.paycrest.io/v1/orders" \
  -H "API-Key: YOUR_CLIENT_ID" \
  -H "Content-Type: application/json" \
  -d '{
    "amount": "100",
...
// Example with PostgreSQL
func saveOrder(db *sql.DB, order map[string]interface{}) error {
    query := `
        INSERT INTO payment_orders (
            id, amount, token, network, status, 
            recipient_institution, recipient_account, recipient_name,
    ...
    recipient := order["recipient"].(map[string]interface{})
